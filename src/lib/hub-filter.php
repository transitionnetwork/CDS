...
function render_hub_filter() {
  $terms = get_terms('hub'); ?>
  <form action="<?php the_permalink() ?>" method="GET" id="hub-filter" class="filter">
  ...
  <strong><?php _e('OR', 'tofino'); ?></strong>
  <?php $terms = get_terms('country'); ?>
  <form action="<?php the_permalink() ?>" method="GET" id="country-filter" class="filter">
    <?php _e('Filter by country:'); ?>
    <select name="country" id="term" onchange="this.form.submit();">
      <option value="">All</option>
      <?php foreach ($terms as $term) { ?>
