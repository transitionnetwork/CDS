  ...
  <div class="mt-6">
    ...
    <?php foreach($co_authors as $user_id) { ?>
      ...
      <div class="mt-2">
        ...
        <form action="<?php the_permalink(); ?>" method="POST" class="inline">
          <input type="hidden" name="ma_post_id" value="<?php echo $post->ID; ?>">
          ...
          <?php } ?>
          <button type="submit" name="ma_remove_co_author_id" value="<?php echo $user_id; ?>" class="btn btn-error btn-sm" onclick="return confirm('Are you sure you want to remove this user?');">Delete</button>
        </form>
      </div>
    <?php } ?>
  </div>
        ...
        <?php foreach($waiting_co_authors as $waiting_co_author) { ?>
          <?php if((int)$waiting_co_author['post_id'] === $post->ID) { ?>
            <div class="mt-2">
              ...
              <form action="<?php the_permalink(); ?>" method="POST" class="inline">
                <button type="submit" name="ma_remove_co_waiting_author_email" value="<?php echo $waiting_co_author['user_email']; ?>" class="btn btn-error btn-sm" onclick="return confirm('Are you sure you want to remove this user?');">Delete</button>
              </form>
            </div>
          <?php } ?>
        <?php } ?>
