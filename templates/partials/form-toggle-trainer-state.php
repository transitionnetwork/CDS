<?php if(is_user_trainer_admin()) { ?>
  <form action="" method="POST" class="mt-3">
    ...
    <?php if(get_post_status() === 'pending' && is_user_trainer_admin()) { ?>
      ...
      <button class="btn btn-danger btn-sm" name="trainer_update" value="trash" onclick="return confirm('<?php echo $confirm_message; ?>')"><?php echo svg('trashcan'); ?>Delete</button>
    <?php } else { ?>
      <button class="btn btn-danger btn-sm" name="trainer_update" value="pending"><?php echo svg('x'); ?>Unpublish</button>
    <?php } ?>
