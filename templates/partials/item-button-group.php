    ...
    <?php if(can_write_initiative($post)) { ?>
      ...
      <?php if(get_post_status($post) === 'publish') { ?>
        ...
        <form action="" method="post" class="inline">
          <button name="unpublish" value="<?php echo $post->ID; ?>" class="btn btn-error btn-sm" onclick="return confirm('<?php echo $confirm_message; ?>')"><?php echo svg('x'); ?><?php _e('Unpublish', 'tofino'); ?></button>
        </form>
      <?php } ?>
    <?php } ?>
