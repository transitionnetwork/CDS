<section>
  <?php if(is_user_role('administrator') || is_user_role('super_hub')) {
    $term_slugs = get_hub_terms();
    foreach($term_slugs as $term_slug) {
      $terms[] = get_term_by('slug', $term_slug, 'hub');
    }
  } else {
    $terms = get_the_terms(wp_get_current_user(), 'hub');
  } ?>
  <div class="row">
    <?php foreach($terms as $term) { ?>
      <div class="col-12">
        <h2><?php _e('Initiative map', 'tofino'); ?> for <a href="<?php echo get_post_type_archive_link('initiatives') . '?term=' . $term->term_id; ?>"><?php echo $term->name; ?></a></h2>
        <?php $url = add_query_arg(array('hub_id' => $term->term_id), get_the_permalink(438));
        ?>
        <iframe id="iframe_map" src="<?php echo $url; ?>"></iframe>
        <div class="panel">
          <p><?php _e('To embed this map, please copy and paste the HTML below', 'tofino'); ?>:</p>
          <pre>&lt;iframe&nbsp;src&#61;&quot;<?php echo $url; ?>&quot;&nbsp;width&#61;&quot;100%&quot;&nbsp;height&#61;&quot;600px&quot;&gt;</pre>
          <p><a href="<?php echo $url ?>" class="btn btn-primary"><?php _e('View Map', 'tofino'); ?></a></p>
        </div>
      </div>
    <?php } ?>
  </div>
</section>
